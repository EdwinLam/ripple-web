...
const vm = avalon.define({
  ...
  openAdd:function(){
    avalon.vmodels['goodAdd'].openInit({afterSave:function(){
      vm.query(1)
    }})
  },
  openEdit:function(el){
    avalon.vmodels['goodEdit'].openInit({afterSave:function(){
      vm.query(1)
    },el:el})
  },
  query: async function (currentPage) {
    const res = await API[API.KEY.GOOD].list({pageNo: currentPage, pageSize: 10})
  ...
  destroy:async function(el){
    COM.confirm({message:'是否删除该数据',afterConfirm: async function () {
      const res = await API[API.KEY.GOOD].destroy(el.id)
      vm.query(1)
    }})
  }
})
