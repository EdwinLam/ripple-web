...
module.exports = class BannerService {
  ...
  static async list (ctx) {
    ...
    const include= [{
      model: db['bannerImage']
    }]
    ctx.body =await SystemUtil.queryPage(bannerDao,ctx.query,pageNo,pageSize,include)
  }
  ...
  static async get(ctx){
    ...
    const data = bannerDao.findById(id,{include})
    if(data){
  ...
  static async save (ctx) {
    ...
    const item = await bannerDao.findOne({where: {nodeId,nodeName}})
    if (item == null) {
      const data = await bannerDao.create(ctx.request.body)
      ctx.body = SystemUtil.createResult({success,message,data})
    } else {
  ...
  static async update (ctx) {
    ...
    await bannerDao.update( {where: {id}})
  ...
  static async delete (ctx) {
    const count = await bannerDao.destroy({where: {id: ctx.params.id}})
